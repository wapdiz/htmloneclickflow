...
function get_stats_today_yesterday(element){
    ...
    if(link.attr('loaded') == 1){
        if(link.attr('show') == 1){
            $('.date_subs_from_'+date).hide();
            link.attr('show','0');
            link.text('Cегодня/вчера');
            
        } else{
            $('.date_subs_from_'+date).show();
            link.attr('show','1');
            link.text('Свернуть');
        }
        ...
        $.ajax({
            url: '/admin/statistic_buyout/get_stats_today_yesterday',
            type: 'GET',
            data: 'date='+date,
...
function get_stats_group_by_split(element){
    ...
    if(link.attr('loaded') == 1){
        ...
        $.ajax({
            url: '/admin/statistic_grupped/get_stats_group_by_split',
            type: 'GET',
...
function get_stats_group_by_param(element){
    ...
    var url = '/admin/statistic_grupped/get_stats_group_by_param_by_date_range';
    if(name_stats == 'subscribe'){
        url = '/admin/statistic_subscribe/get_stats_group_by_param_by_date_range';
    }else if(name_stats == 'buyout'){        
        url = '/admin/statistic_buyout/get_stats_group_by_param_by_date_range';
    }    
    ...
    if($(element).attr('customWhereParams') == 1){
        ...
        var User = $('#customFilter_'+name_stats+'_User').val();
        ...
        $.ajax({
            ...
            +'&User='+User+'&Landings='+Landings+'&Credit='+Credit,
            beforeSend: function(){
                $('#stats_content_'+name_stats).html('<tr><td colspan="17" style="text-align:center;"><img src="/assets/images/load.gif"></td></tr>');
            },
            success: function(msg){
                //$('#stats_content_'+name_stats).html(start);
...
function get_stats_tab_content(element, name_stats){
    ...
    var url = '/admin/statistic_grupped/get_stats_tab_content';
    if(name_stats == 'subscribe'){
        url = '/admin/statistic_subscribe/get_stats_tab_content';
    }else if(name_stats == 'buyout'){        
        url = '/admin/statistic_buyout/get_stats_tab_content';
    }
    ...
    $.ajax({
        ...
        success: function(msg){            
            $('#'+name_stats).html(msg);
            $('#'+name_stats).queue(function (next) {           
                $('#customFilter_'+name_stats+'_User').chosen();                    
                next();
            });
        },
        error: function(){            
