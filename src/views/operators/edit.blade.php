        ...
        <form method="POST" action="/admin/operators/store" id="operator-form" class="form-horizontal form-bordered">
            <section class="panel">
                <header class="panel-heading">
                    <h2>Редактирование оператора {{$operator['name']}}</h2>
                </header>
                <div class="panel-body">
                        ...
                        <div class="col-md-6">
                            <select class="form-control" name="country" id="country">
                                @foreach($country as $item)
                                    <option value="{{$item['country_id']}}" @if($operator['country'] == $item['country_id']) selected @endif >{{$item['name']}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-6">
                            <textarea class="form-control" name="ip" rows="3" id="textareaDefault">@foreach($operator_ip as $item){{long2ip($item['start'])}}-{{long2ip($item['end'])}}&#13;&#10;@endforeach</textarea>
                        </div>
                    </div>
